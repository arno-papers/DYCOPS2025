...
include("cstr_definitions.jl")

# first experiment

optimization_state =  zeros(15)
optimization_initial = optimization_state[1]
@mtkbuild true_cstr = TrueCSTR()
prob = ODEProblem(true_cstr, [], (0.0, 15.0), [], tstops = 0:15, save_everystep=false)
sol = solve(prob, Rodas5P())

@mtkbuild  ude_cstr = UDECSTR()
ude_prob = ODEProblem(ude_cstr, [], (0.0, 15.0), [], tstops = 0:15, save_everystep=false)
...
x0 = reduce(vcat, getindex.((default_values(ude_cstr),), tunable_parameters(ude_cstr)))
get_vars = getu(ude_cstr, [ude_cstr.C_s])
...
extracted_chain = arguments(equations(ude_cstr.nn)[1].rhs)[1]
T = defaults(ude_cstr)[ude_cstr.nn.T]
C_s_range_plot = 0.0:0.01:20.0
...
# inhibition curve for reference
μ_inhibition(C_s) = μ_max * C_s / (K_s + C_s + C_s^2 / K_i)

plts = plot(), plot(), plot()
for i in 1:length(model_structures)
    plot!(plts[3],  C_s_range_plot, model_structures[i].( C_s_range_plot);c=i+2,lw=1,ls=:dash)
...
plot!(plts[3], C_s_range_plot, μ_inhibition.(C_s_range_plot), lw=3, c=1)
plot!(plts[3], C_s_range_plot, μ_predicted_plot, lw=3, c=2)
scatter!(plts[3], data[!, "C_s(t)"], μ_predicted_data, ms=3, c=2)
plot!(plts[3], ylabel="μ(1/h)", xlabel="Cₛ(g/L)",ylims=(0,0.7))
plot(plts..., layout = 3, tickfontsize=10, guidefontsize=12, legendfontsize=14, grid=false, legend=false)
savefig("cstr_experiment1.pdf")
...
plot!(plts[1], t_pwc, pwc, lw=3, color=:black,xlabel="t(h)",ylabel="Qin(1/h)")
for i in 1:length(model_structures)
    plausible_prob = probs_plausible[i]
...
savefig("cstr_control1.pdf")

# second experiment
@mtkbuild true_cstr2 = TrueCSTR()
prob2 = ODEProblem(true_cstr2, [], (0.0, 15.0), [], tstops=0:15, save_everystep=false)
sol2 = solve(prob2, Rodas5P())
@mtkbuild ude_cstr2 = UDECSTR()
ude_prob2 = ODEProblem(ude_cstr2, [], (0.0, 15.0), [ude_cstr2.Q_in => optimization_initial], tstops=0:15, save_everystep=false)
ude_sol2 = solve(ude_prob2, Rodas5P())
ude_prob_remake = remake(ude_prob, p=ude_prob2.p)
sol_remake = solve(ude_prob_remake, Rodas5P())
x0 = reduce(vcat, getindex.((default_values(ude_cstr),), tunable_parameters(ude_cstr)))

get_vars2 = getu(ude_cstr2, [ude_cstr2.C_s])
...
extracted_chain = arguments(equations(ude_cstr2.nn)[1].rhs)[1]
T = defaults(ude_cstr2)[ude_cstr2.nn.T]
...
plot!(plts[2], ylabel="Cₓ(g/L)", xlabel="t(h)")
plot!(plts[3], C_s_range_plot, μ_inhibition.(C_s_range_plot), lw=3, c=1)
plot!(plts[3], C_s_range_plot, μ_predicted_plot2, lw=3, c=2)
scatter!(plts[3], data[!, "C_s(t)"], μ_predicted_data, ms=3, c=2)
scatter!(plts[3], data2[!, "C_s(t)"], μ_predicted_data2, ms=3, c=2)
plot!(plts[3], ylabel="μ(1/h)", xlabel="Cₛ(g/L)",ylims=(0,0.7))
plot(plts..., layout = 3, tickfontsize=10, guidefontsize=12, legendfontsize=14, grid=false, legend=false)
savefig("cstr_experiment2.pdf")
...
plot!(plts[2],xlabel="t(h)",ylabel="Cₛ(g/L)")
plot(plts..., layout = (2, 1), tickfontsize=12, guidefontsize=14, legendfontsize=14, grid=false, legend=false)
savefig("cstr_control2.pdf")


# third experiment
@mtkbuild true_cstr3 = TrueCSTR()
prob3 = ODEProblem(true_cstr3, [], (0.0, 15.0), [], tstops=0:15, save_everystep=false)
sol3 = solve(prob3, Rodas5P())
@mtkbuild ude_cstr3 = UDECSTR()
ude_prob3 = ODEProblem(ude_cstr3, [], (0.0, 15.0), tstops=0:15, save_everystep=false)

x0 = reduce(vcat, getindex.((default_values(ude_cstr3),), tunable_parameters(ude_cstr3)))

get_vars3 = getu(ude_cstr3, [ude_cstr3.C_s])
...
## get chain from the equations
extracted_chain = arguments(equations(ude_cstr3.nn)[1].rhs)[1]
T = defaults(ude_cstr3)[ude_cstr3.nn.T]
...
plot!(C_s_range_plot, μ_inhibition.(C_s_range_plot), lw=3, c=1)
...
plot!(ylabel="μ(1/h)", xlabel="Cₛ(g/L)",ylims=(0,0.7),legend=false)
