...
function run_comparison_experiment(control_strategy::Symbol, experiment_id::Int)
    # Generate 3 sets of controls based on strategy
    if control_strategy == :random
        optimization_state_1 = rand(15) * 0.5
        optimization_state_2 = rand(15) * 0.5
        optimization_state_3 = rand(15) * 0.5
    elseif control_strategy == :uniform_grid
        optimization_state_1 = collect(range(0.0, 0.5, length=15))
        optimization_state_2 = collect(range(0.5, 0.0, length=15))
        optimization_state_3 = collect(range(0.0, 0.5, length=15))
    end

    optimization_state = optimization_state_1
    optimization_initial = optimization_state[1]
    @mtkbuild true_cstr1 = TrueCSTR()
    prob1 = ODEProblem(true_cstr1, [], (0.0, 15.0), [], tstops = 0:15, save_everystep=false)
    @mtkbuild ude_cstr1 = UDECSTR()
    ude_prob1 = ODEProblem(ude_cstr1, [], (0.0, 15.0), [], tstops = 0:15, save_everystep=false)

    optimization_state = optimization_state_2
    optimization_initial = optimization_state[1]
    ...
    ude_prob2 = ODEProblem(ude_cstr2, [], (0.0, 15.0), [], tstops = 0:15, save_everystep=false)

    optimization_state = optimization_state_3
    optimization_initial = optimization_state[1]
    ...
    get_vars1 = getu(ude_cstr1, [ude_cstr1.C_s])
    ...
    x0 = reduce(vcat, getindex.((default_values(ude_cstr1),), tunable_parameters(ude_cstr1)))
    ...
    extracted_chain = arguments(equations(ude_cstr1.nn)[1].rhs)[1]
    T = defaults(ude_cstr1)[ude_cstr1.nn.T]
    ...
    println("=== $(control_strategy) experiment $(experiment_id) ===")
...
println("\n========== RANDOM EXPERIMENTS ==========")
for i in 1:5
    run_comparison_experiment(:random, i)
end

# Run 5 uniform grid experiments
println("\n========== UNIFORM GRID EXPERIMENTS ==========")
for i in 1:5
    run_comparison_experiment(:uniform_grid, i)
end
